...
#define B_FLAG_INVERSE_BATTLE       FLAG_INVERSE_BATTLE        // 0x20; If this flag is set, the battle's type effectiveness are inversed. For example, fire is super effective against water.
#define B_FLAG_FORCE_DOUBLE_WILD    FLAG_FORCE_DOUBLE_WILD     // 0x21; If this flag is set, all land and surfing wild battles will be double battles.
#define B_SMART_WILD_AI_FLAG        SMART_WILD_AI_FLAG         // 0x22; If not 0, you can set this flag in a script to enable smart wild pokemon
#define B_FLAG_NO_BAG_USE           FLAG_NO_BAG_USE            // 0x23; If this flag is set, the ability to use the bag in battle is disabled.
#define B_FLAG_NO_CATCHING          FLAG_NO_CATCHING           // 0x24; If this flag is set, the ability to catch wild Pokémon is disabled.
#define B_FLAG_NO_RUNNING           FLAG_NO_RUNNING            // 0x25; If this flag is set, the ability to escape from wild battles is disabled. Also makes Roar/Whirlwind and Teleport (under Gen8) fail.
#define B_FLAG_AI_VS_AI_BATTLE      FLAG_AI_VS_AI_BATTLE       // 0x26; If this flag is set, the player's mons will be controlled by the ai next battles.
#define B_FLAG_DYNAMAX_BATTLE       FLAG_DYNAMAX_BATTLE        // 0x27; If this flag is set, the ability to Dynamax in battle is enabled for all trainers.
#define B_FLAG_TERA_ORB_CHARGED     FLAG_TERA_ORB_CHARGED      // 0x28; If this flag is set, the Tera Orb is charged. It is automatically set upon healing and cleared upon Terastallizing once configured.
#define B_FLAG_TERA_ORB_NO_COST     FLAG_TERA_ORB_NO_COST      // 0x29; If this flag is set, the Tera Orb does not use up its charge upon Terastallization. In S/V, this occurs after an event with Terapagos.
#define B_FLAG_SLEEP_CLAUSE         FLAG_SLEEP_CLAUSE          // 0x2A; If this flag is set, sleep clause is enabled; if the player / AI has already put a Pokémon on the opponent's side to sleep and it is still sleeping, another one can't be put to sleep. AI requires AI_FLAG_CHECK_BAD_MOVE to understand.

// Var Settings
// To use the following features in scripting, replace the 0s with the var ID you're assigning it to.
// Eg: Replace with VAR_UNUSED_0x40F7 so you can use B_VAR_STARTING_STATUS for that feature.
#define B_VAR_STARTING_STATUS       VAR_STARTING_STATUS        // 0x404E; If this var has a value, assigning a STATUS_FIELD_xx_TERRAIN to it before battle causes the battle to start with that terrain active.
#define B_VAR_STARTING_STATUS_TIMER VAR_STARTING_STATUS_TIMER  // 0x4083; If this var has a value greater or equal than 1 field terrains will last that number of turns, otherwise they will last until they're overwritten.
#define B_VAR_WILD_AI_FLAGS         VAR_WILD_AI_FLAGS          // 0x408B; If not 0, you can use this var to add to default wild AI flags. NOT usable with flags above (1 << 15)
#define B_VAR_DIFFICULTY            VAR_DIFFICULTY             // 0x4091; If not 0, you can use this var to control which difficulty version of a Trainer is loaded. This should be manually set by the developer using Script_SetDifficulty AFTER NewGameInitData has run.

// Sky Battles
#define B_FLAG_SKY_BATTLE                 FLAG_SKY_BATTLE     // 0x2B; If this flag has a value, the player will be able to engage in scripted Sky Battles.
#define B_VAR_SKY_BATTLE                  VAR_SKY_BATTLE      // 0x409B; If this var has a value, the game will remember the positions of Pokémon used in Sky Battles.
