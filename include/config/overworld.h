...
#define OW_PC_RELEASE_ITEM          GEN_7      //* In Gen8+, Pokémon that get released with a held item return it to the bag.

#define OW_BERRY_MUTATIONS             TRUE       //* If enabled, Berry plants can mutate based on berries planted next to them.
#define OW_BERRY_MUTATION_CHANCE       20         //* Determines the % chance of a mutation, default is 25%
...
#define B_FLAG_FOLLOWERS_DISABLED      FLAG_FOLLOWERS_DISABLED  // 0x2D; Enables / Disables followers by using a flag. Helpful to disable followers for a period of time.
...
#define OW_FLAG_PAUSE_TIME          FLAG_PAUSE_TIME      // 0x2E; If this flag is set and OW_USE_FAKE_RTC is enabled, seconds on the in-game clock will not advance.
#define OW_FLAG_NO_ENCOUNTER        FLAG_NO_ENCOUNTER    // 0x2F; If this flag is set, wild encounters will be disabled.
#define OW_FLAG_NO_TRAINER_SEE      FLAG_NO_TRAINER_SEE  // 0x30; If this flag is set, trainers will not battle the player unless they're talked to.
#define OW_FLAG_NO_COLLISION        FLAG_NO_COLLISION    // 0x31; If this flag is set, the player will be able to walk over tiles with collision. Mainly intended for debugging purposes.
#define OW_FLAG_POKE_RIDER          FLAG_POKE_RIDER      // 0x32; If this flag is set, the player will be able to use fly from the Pokenav Region Map and the Town Map key item by pressing 'R' on a city/location they are able to fly to.
...
#define OW_FLAG_MOVE_UNION_ROOM_CHECK    FLAG_UNION_ROOM_CHECK   // 0x33; If this flag is set, the game will only check if players are in the Union Room while healing Pokémon, and not when players enter the Pokémon Center. This speeds up the loading of the Pokémon Center. This is ignored if OW_UNION_DISABLE_CHECK is TRUE.
// Berry Blender
